...
list_random_number=[]
def random_namber(dim, N):
    for i in range(dim):
        list_random_number.append(numpy.random.uniform(0, 1, N))
...
def selector(algo, func_details, popSize, Iter):
    ...
    ub = func_details[2]
    dim = func_details[3]
    random_namber(dim, popSize)
    #print("list_random_number::::",list_random_number)
    if algo == "RA":
        x = ra.RA(getattr(benchmarks, function_name), lb, ub, dim, popSize, Iter)
    else:
        return None
    return x
